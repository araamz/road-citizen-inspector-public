...
export default async function getDeviceVisualization(deviceId: number, queryParams: DeviceAbstractQuerySchema): Promise<DeviceVisualizationData> {
    ...
    const response = await fetch(`${serverUrl()}/visualization/device/${deviceId}/summary?${queryUrl.toString()}`, {
        credentials: 'include'
    })
    ...
    if (!success) throw new VisualizationError('Error occurred getting device visualization. Data maybe malformed or unavailable.')
