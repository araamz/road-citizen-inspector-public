...
import type { ProjectReadingsContract } from "@road-citizen-inspector/contracts";
import type { ReadingQuerySchema } from "@road-citizen-inspector/schemas";

export default async function getProjectReadings(queryParams: ReadingQuerySchema) {
    ...
    const response = await fetch(`${serverUrl()}/reading/project?${queryUrl.toString()}`, {
        method: "GET",
        credentials: "include"
    })
    ...
    const { success, data } = (await response.json()) as ProjectReadingsContract
