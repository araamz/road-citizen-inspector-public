...
import type { ProjectStatusContract } from "@road-citizen-inspector/contracts";
import type { StatusQueryFormSchema } from "@/schemas/StatusQueryForm.schema";

export default async function getProjectStatus(queryParams: StatusQueryFormSchema) {
    ...
    const response = await fetch(`${serverUrl()}/status/project?${queryUrl.toString()}`, {
        method: "GET",
        headers: {
    ...
    const { success, data } = await (response.json()) as ProjectStatusContract;
